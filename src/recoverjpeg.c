...
static void usage(int clean_exit) {
...
static inline int progressbar() { return !(quiet || verbose); }

static void display_progressbar(off_t offset, unsigned int n) {
  off_t to_display;
  ...
  if (n != old_n || to_display != old_to_display) {
    printf("\rRecovered files: %4u        Analyzed: %4.1f %s  ", n,
           to_display / 10.0, gib_mode ? "GiB" : "MiB");
    fflush(stdout);
    old_n = n;
...
static void cleanup_progressbar() {
  printf("\r                                                     \r");
}

static size_t jpeg_size(const unsigned char *start) {
  ...
  for (addr = start + 2;;) {
    if (*addr != 0xff) {
      if (verbose) {
        fprintf(stderr, "   Incorrect marker %02x, stopping prematurely\n",
                *addr);
      }
      return 0;
    }
    ...
    if (code == 0xd9) {
      if (verbose) {
        fprintf(stderr, "   Found end of image after %ld bytes\n",
                (long)(addr - start + 1));
      }
      return addr - start;
    }
    ...
    if (code == 0xda) {
      ...
      for (; (size_t)(addr - start) < max_size &&
             (*addr != 0xff || *(addr + 1) == 0 || /* Escape */
...
int main(int argc, const char *const argv[]) {
  ...
  if (fd < 0) {
    fprintf(stderr, "recoverjpeg: unable to open %s for reading (%s)\n",
  ...
  if (start == 0) {
    fprintf(stderr, "recoverjpeg: cannot allocate necessary memory (%s)\n",
            strerror(errno));
    exit(1);
  }
